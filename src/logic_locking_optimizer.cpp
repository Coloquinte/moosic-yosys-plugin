...
LogicLockingOptimizer::LogicLockingOptimizer(
    const std::vector<std::vector<int>> &pairwiseInterference)
    ...
    for (int node : c) {
      if (node >= nbNodes() || node < 0) {
        throw std::runtime_error(
            "Solution is invalid: some nodes are out of bound");
      }
      auto [it, inserted] = present.insert(node);
      if (!inserted) {
        throw std::runtime_error(
            "Solution is invalid: same node is present in multiple groups");
      }
    }
    if (!isClique(c)) {
      throw std::runtime_error(
          "Solution is invalid: some groups are not cliques");
    }
  }
}
...
void LogicLockingOptimizer::check() const {
  for (int i = 0; i < (int) pairwiseInterference_.size(); ++i) {
    ...
    for (int j : v) {
      if (i == j) {
        throw std::runtime_error(
            "Pairwise interference is invalid: should have no self-loop");
      }
      if (j < 0 || j >= nbNodes()) {
        throw std::runtime_error(
            "Pairwise interference is invalid: some nodes are out of bound");
      }
      if (!hasEdge(j, i)) {
        throw std::runtime_error(
            "Pairwise interference is invalid: reverse edge is not present");
      }
    }
    if (!std::is_sorted(v.begin(), v.end())) {
      throw std::runtime_error(
          "Pairwise interference is invalid: should be sorted for each node");
    }
    if (std::adjacent_find(v.begin(), v.end()) != v.end()) {
      throw std::runtime_error(
          "Pairwise interference is invalid: should have no duplicate");
    }
  }
}
...
std::vector<std::vector<int>> LogicLockingOptimizer::listMaximalCliques()
    const {
  std::vector<int> P;
...
void LogicLockingOptimizer::bronKerbosch(
...
LogicLockingOptimizer::ExplicitSolution LogicLockingOptimizer::solveBruteForce(
    int maxNumber) const {
  int currentNumber = 0;
...
LogicLockingOptimizer::Solution LogicLockingOptimizer::flattenSolution(
    const ExplicitSolution &sol) {
  std::vector<int> ret;
  for (const auto &c : sol) {
    for (int i : c) {
      ret.push_back(i);
